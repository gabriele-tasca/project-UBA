...
export var mouse_input = MOUSE_INPUT.withdraw
...
const YELLOW_BANG = preload("res://scenes/vfx/yellow-bang.tscn")
...
export var target_b_pos: Vector2
...
# or hit the ground
onready var body_radius = $body/CollisionShape2D.shape.radius
func _on_body_body_entered(collider):
	if collider.is_in_group("hand"):
		...
		if coll_atk_disabled == false && collider.offensive_arc && collider.linear_velocity.length() > real_punch_min_speed:
			...
			var surface_normal = ($body.global_position - collider.global_position).normalized()
			yellow1.global_position = $body.global_position - surface_normal*body_radius
			yellow1.rotation = surface_normal.angle()

				
			
	if net_mode == NET_MODE.own_on_host or net_mode == NET_MODE.other_on_host:
		if collider.is_in_group("arena"):
			# bounce adjust
...
#			var VFXB = preload("res://character/vfx-bounce/vfx-bounce.tscn")
#			var vfxb = VFXB.instance()
#			get_parent().add_child(vfxb)
#			vfxb.global_position = $body.global_position
...
func set_color(color: Color):
	$body/g/Sprite.modulate = Color(color).lightened(0.3)
	$hand/g/Sprite.modulate = Color(color).lightened(0.3)

func set_color2(color: Color):
	$body/g/SpriteColor.modulate = Color(color)
...
func set_character_data(c_data):
	...
	set_color2(Color(c_data.color2))
